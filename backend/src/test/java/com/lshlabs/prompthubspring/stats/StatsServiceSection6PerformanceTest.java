...
class StatsServiceSection6PerformanceTest {
    ...
    void dashboardCache_reportsHitRatioAndWarmP95() {
        Platform platform = savePlatform("section6_stats_perf_platform");
        Category category = saveCategory("section6_stats_perf_category");
        AppUser author = saveUser("section6_stats_perf_author");
        AppUser actor = saveUser("section6_stats_perf_actor");

        for (int i = 0; i < 80; i++) {
            Post p = savePost(author, platform, category, "통계성능-" + i, i);
            if (i % 3 == 0) {
                saveInteraction(actor, p);
            }
        }
        ...
        System.out.printf("SECTION6_STATS_DASHBOARD_CACHE_METRIC hit_ratio=%.4f cold_ms=%.2f warm_p95_ms=%.2f warm_calls=%d%n",
    ...
    void userStats_reportsTpsAndP95Metrics() {
        Platform platform = savePlatform("section6_userstats_platform");
        Category category = saveCategory("section6_userstats_category");
        AppUser author = saveUser("section6_userstats_author");
        AppUser actor = saveUser("section6_userstats_actor");

        for (int i = 0; i < 100; i++) {
            Post p = savePost(author, platform, category, "유저통계성능-" + i, i);
            if (i % 2 == 0) {
                saveInteraction(actor, p);
            }
        }
        ...
        System.out.printf("SECTION6_STATS_USER_METRIC tps=%.2f p95_ms=%.2f iterations=%d%n", tps, p95Millis, iterations);
    ...
    private Post savePost(AppUser author, Platform platform, Category category, String title, int offset) {
        ...
        post.setTitle(title);
        post.setPrompt("section6 stats performance prompt 본문입니다.");
        post.setAiResponse("section6 stats performance ai_response 본문입니다.");
