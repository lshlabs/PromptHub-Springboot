...
@SpringBootTest
@ActiveProfiles("test")
class CoreServiceSection6PerformanceTest {
    ...
    void search_reportsTpsAndP95Metrics() {
        Platform platform = savePlatform("section6_perf_platform");
        Category category = saveCategory("section6_perf_category");
        AppUser author = saveUser("section6_perf_author");
        for (int i = 0; i < 120; i++) {
    ...
    void trendingCache_reportsHitRatioAndWarmP95() {
        Platform platform = savePlatform("section6_cache_platform");
        Category category = saveCategory("section6_cache_category");
        AppUser author = saveUser("section6_cache_author");
        for (int i = 0; i < 100; i++) {
    ...
    private void savePost(AppUser author, Platform platform, Category category, String title, BigDecimal satisfaction) {
        ...
        post.setTitle(title);
        post.setPrompt("section6 performance prompt 본문입니다.");
        post.setAiResponse("section6 performance ai_response 본문입니다.");
        post.setSatisfaction(satisfaction);
        post.setTags("perf,section6");
